...
list <-  split(var2a, seq(nrow(dataframe)))
names(coin_counts) <- list
return(coin_counts)
...
sumatory <- lapply(mylist, function(x) sum(x))
n <- unlist(sumatory)
vector_ratius <- as.vector(df['radius'])
...
list_of_coins <- Distribution(143, df)
...
coin_types <- c(Gold, Copper, Silver)
