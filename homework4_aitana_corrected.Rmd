...
```{r}
library(data.table)
file_list1 <- list.files(path = "../Task1/Healthy", #paste0(getwd(), "/Task1/Healthy"), 
                         full.names=T)
#combined_files_h <- do.call("rbind", lapply(file_list1, read.csv))
file_list2 <- list.files(path = "../Task1/Severe_kidney_disease", #path = paste0(getwd(), "/Task1/Severe_kidney_disease"), 
                         full.names=T)
...
<span style="color:red"> 2.5/5 pts Formating of the table is not correct. Groups should be in column names and not the elements of the columns. Also you needed to calculate individually for three patients with the highest intensity. Finding those 3 patients and reporting their calculations. You can use the ordinary table trick to remove first row and add it as the column names or awesome function transpose from data.table instead of t(). </span>
...
```
<span style="color:red"> 2/4 Not completely correct sentence: THE ONES THAT ARE TRUE FOR ALL THE SPECIES ARE THE ONES THAT HAVE TRUE IN THE COLUMN OVER_UNDER 5 TIMES. You mean 7 times because there are 7 sponges. Also, the changes are close to 0 but what if the expected_freq was equal to calculated value? You have to take that into an account. Better examination of the final table</span>
...
<span style="color:red"> 2/5 Comments on the results as well as biological explanation are missing. What about those under score 0?</span>
...
SBS2 <- readRDS("../Task3/SBS2.RDS")
SBS13 <- readRDS("../Task3/SBS13.RDS")
...
column_intervals <- fread("../Task3/intervals.txt")
column_special_names <- fread("../Task3/interval_names.txt"); column_special_names
patients <- fread("../Task3/patients.txt")
...
```{r}
patients_cancer_type <- fread("../Task3/patientsCancerType.txt"); patients_cancer_type
...
# Corrected by PS
cancer_types <- unlist(lapply(pat, function(x) paste(x, patients_cancer_type[, cancer_type[variable == x]], "")))#unlist(lapply(pat, function(x) paste(x, patients_cancer_type[, V2[V1 == x]], ""))); cancer_types # PS V2 does not exist. You have changed it to cancer_type. 
...
```
<span style="color:red"> 1.75/3 it is nice that you are not changing the column type by adding a new row. However, this type of adding information is not tidy. In this task you were asked to work as tidy as you can.  </span>
...
```
<span style="color:green"> 2.5/2 Why haven't you submitted this melt solution to the previous task since it perfectly solve all the issues you may be facing working with the previously non-tidy generated table? You are working breast adenocarcinoma as it is stated here and in the goal of this task. The sumatorys table contains all the patients when it should contain only  Breast-AdenoCA. You get the correct solution since SBS2 and SBS13 are most abundant and present in breast cancer and often not in other type of cancers. Because you used melt here instead of the previous task you are getting one extra point (+1) </span>
